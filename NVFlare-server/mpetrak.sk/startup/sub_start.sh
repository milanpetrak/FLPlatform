...
start_fl() {
  ...
  lst=$SECONDS
((python3 -u -m nvflare.private.fed.app.server.server_train -m $DIR/.. -s fed_server.json --set secure_train=true org=ukf config_folder=config 2>&1 & echo $! >&3 ) 3>$DIR/../pid.fl )
  pid=`cat $DIR/../pid.fl`
  echo "new pid ${pid}"
}
